...
def main():
    ...
    with p.option_context('display.max_rows', 4000, 'display.max_columns', 4000, 'display.width', 1000000):
        df = df.ix[:, 'side lots lotsEtoro lotsEtoroRT lotsEtoroRT2 boolLotsEtoroRT2 diffpLotsEtoro minimumLeverageEtoro unitsEtoro balanceMetatrader leverageMetatrader allMarginMetatrader amount2Metatrader closeTradePLMetatrader side diffp lots'.split(' ')]
        #df = df.sort_index()
